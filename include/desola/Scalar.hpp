...
#ifndef DESOLA_SCALAR_HPP
#define DESOLA_SCALAR_HPP
...
template<typename T_element>
const desola::Scalar<T_element> abs(const desola::Scalar<T_element>& operand)
{
  return operand.abs();
}

template<typename T_element>
const desola::Scalar<T_element> sqrt(const desola::Scalar<T_element>& operand)
{
  return operand.sqrt();
}
